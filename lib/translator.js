...
module.exports = (function() {
  var Translator = function(options){
    ...
    this.useDefaultTransformer = !!options.pattern;
  ...
  Translator.prototype.translate = function(content) {
    ...
    resultPromise.resolve(content.replace(self.pattern, function (s, data) {
      ...
      /**
       * Transform the input string with the specified transform function.
       * This function can be used in cascade function (e.g. Array#reduce).
       * The actual transform function to used is based on the existence of 
       * the userTransform option and the pre-defined base transform function.
       *
       * @method
       * @private
       * @param {String|Error} res  The string to transform, or the cascaded error
       * @param {String} transformName  The name of the transform function
       * @returns {String|Error} The transformed string, or error
       */
      var transform = function(res, transformName) {
      ...
      if (transforms.length === 0 && self.useDefaultTransformer) {
        // no transform modifier was specified, and custom pattern was defined
        // transform using "translate"
        res = transform(value, 'translate');
      } else {
        // transform using the transform functions in the array
        res = transforms.reduce(transform, value);
      }
